...
def wait_for_menu(window_id: str, probe_name: str, attempts: int = 30, delay: float = 0.25) -> Path:
    ...
    for attempt in range(attempts):
        ...
        if visible:
            ...
            return ready
        time.sleep(delay)
    raise AssertionError(f"menu not detected for {probe_name}")
...
def click_detected_menu_button(window_id: str, probe_image: Path, role: str, output_prefix: str) -> None:
    ...
    activate_window(window_id)
    time.sleep(0.1)
    run_cmd(["xdotool", "mousemove", "--window", window_id, str(target[0]), str(target[1])])
...
def wait_for_menu_to_disappear(window_id: str, output_prefix: str, attempts: int = 20, delay: float = 0.25) -> None:
    ...
    for attempt in range(attempts):
        ...
        if not still_visible:
            return
        time.sleep(delay)
    raise AssertionError(f"menu still visible for {output_prefix}")
...
def start_xvfb() -> None:
    ...
    X11_ENV = {
        **os.environ,
        ...
        "UI_TEST_SCENARIO": "inventory_proximity",
...
def wait_for_runtime_windows(expected_count: int) -> list[str]:
    for _ in range(120):
        ...
        time.sleep(0.25)
    raise AssertionError(f"expected {expected_count} runtime windows")
...
def _result_path(role: str) -> Path:
    return OUT_DIR / f"inventory_proximity_{role}.json"


def _wait_result(role: str, timeout_sec: float = 6.0) -> dict:
    deadline = time.monotonic() + timeout_sec
    path = _result_path(role)
    while time.monotonic() < deadline:
        if path.exists():
            try:
                ...
                pass
        time.sleep(0.2)
    raise AssertionError(f"result file not found for role={role}: {path}")


def main() -> int:
    # Clean previous run artifacts.
    for old_file in OUT_DIR.iterdir():
        if old_file.is_file() and (old_file.suffix.lower() in {".png", ".log", ".txt", ".json"}):
            old_file.unlink()
    ...
    phase("Attente résultat", "client_a écrit inventory_proximity_client_a.json")
    result = _wait_result("client_a")
    near_ok = bool(result.get("near_has_target", False))
    near_name = str(result.get("near_target_name", ""))
    far_has = bool(result.get("far_has_target", True))
    far_name = str(result.get("far_target_name", ""))
    far_distance = float(result.get("far_distance", 0.0))
    chest_ok = bool(result.get("chest_has_target", False))
    chest_name = str(result.get("chest_target_name", ""))

    if not near_ok or near_name != "Sac":
        raise AssertionError(f"Devrait cibler le joueur proche (Sac). Résultat: {result}")
    if far_distance < 30.0:
        raise AssertionError(f"Setup invalide: distance loin trop faible ({far_distance}). Résultat: {result}")
    if far_has or far_name != "":
        raise AssertionError(f"Ne devrait PAS cibler le joueur loin. Résultat: {result}")
    if not chest_ok or "Coffre" not in chest_name:
        raise AssertionError(f"Devrait cibler le coffre quand devant. Résultat: {result}")

    phase("Assertion", f"OK near=({near_ok},{near_name}) far=({far_has},{far_name},dist={far_distance:.1f}) chest=({chest_ok},{chest_name})")
    return 0
