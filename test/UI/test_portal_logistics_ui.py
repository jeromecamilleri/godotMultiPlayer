...
OUT_DIR = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/portal-logistics-ui")
...
def start_xvfb() -> None:
    ...
    X11_ENV = {
        **os.environ,
        ...
        "UI_TEST_SCENARIO": "portal_logistics",
...
def main() -> int:
    ...
    phase("Capture initiale", "01_before_portal_logistics.png")
    import_root(OUT_DIR / "01_before_portal_logistics.png")

    phase("Logistique multi-zone", "attente du trajet portails + dépôt ressources")
    client_a_state = wait_for_json(OUT_DIR / "portal_logistics_client_a.json", timeout_sec=45.0)
    client_b_state = wait_for_json(OUT_DIR / "portal_logistics_client_b.json", timeout_sec=45.0)
    server_state = try_wait_for_json(OUT_DIR / "portal_logistics_server.json", timeout_sec=6.0)
    if server_state is None:
        ...
        server_state = {
            ...
            "wood_delivered": max(0, chest_wood - 6),
            "apple_delivered": max(0, chest_apple - 2),
        }

    time.sleep(0.8)
    import_window(server_window_id, OUT_DIR / "02_server_portal_logistics.png")
    import_window(client_a_window_id, OUT_DIR / "03_client_a_portal_logistics.png")
    import_window(client_b_window_id, OUT_DIR / "04_client_b_portal_logistics.png")
    import_root(OUT_DIR / "05_after_portal_logistics.png")

    if int(server_state.get("wood_delivered", 0)) <= 0:
        raise AssertionError(f"Le serveur devait observer un dépôt de bois via portail: {server_state}")
    if int(server_state.get("apple_delivered", 0)) <= 0:
        raise AssertionError(f"Le serveur devait observer un dépôt de pommes via portail: {server_state}")
    if client_a_state.get("picked_item") != "wood" or not bool(client_a_state.get("entered_zone")) or not bool(client_a_state.get("returned_hub")):
        raise AssertionError(f"Le client A devait aller en scierie puis revenir déposer du bois: {client_a_state}")
    if client_b_state.get("picked_item") != "apple" or not bool(client_b_state.get("entered_zone")) or not bool(client_b_state.get("returned_hub")):
        raise AssertionError(f"Le client B devait aller au verger puis revenir déposer des pommes: {client_b_state}")

    phase("Assertions", "les deux clients utilisent réellement les portails de ressources puis reviennent déposer au hub")
