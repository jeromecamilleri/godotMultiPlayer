...
OUT_DIR = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("/tmp/inventory-ui-transfer")
...
for old_file in OUT_DIR.iterdir():
    if not old_file.is_file():
        continue
    if (
        old_file.suffix.lower() == ".png"
        or old_file.name in {"run.log", "summary.txt", "xvfb.log"}
        or (old_file.suffix.lower() == ".json" and old_file.name.startswith("chest_"))
    ):
        old_file.unlink()
...
def _get_chest_item_quantity(contents: list, item_id: str) -> int:
    """Retourne la quantité totale pour item_id dans la liste de slots (contenu coffre)."""
    total = 0
...
def assert_chest_apple_count(role: str, expected: int, msg: str = "") -> None:
    """Vérifie que le coffre vu par ce client a exactement expected pommes (apple)."""
    contents = _wait_chest_file(role)
    if contents is None:
        raise AssertionError(f"Fichier coffre non trouvé pour {role} (synchro non écrite). {msg}")
    qty = _get_chest_item_quantity(contents, "apple")
    if qty != expected:
        raise AssertionError(
            f"Coffre {role}: attendu Pomme x{expected}, vu Pomme x{qty}. {msg}"
        )
    log(f"assert_chest_apple_count OK: {role} Pomme x{qty}")


def assert_chest_sync_both_clients(expected_apple: int) -> None:
    """Vérifie que client_a et client_b voient le même contenu de coffre (Pomme x expected)."""
    assert_chest_apple_count("client_a", expected_apple, "Vue client_a après transfert.")
    assert_chest_apple_count("client_b", expected_apple, "Vue client_b après ouverture coffre (synchro).")
    log("assert_chest_sync_both_clients OK: les deux clients voient le même coffre.")
...
def main() -> int:
    ...
    phase("Etat initial", "05_initial_observation.png")
    import_root(OUT_DIR / "05_initial_observation.png")

    phase("Client A ramasse la pomme", "scenario deterministe de pickup puis ouverture du coffre")
    time.sleep(2.0)
    import_root(OUT_DIR / "06_after_client_a_pickup.png")

    phase("Client A transfère au coffre", "scenario deterministe de transfert vers le coffre")
    time.sleep(2.0)
    import_root(OUT_DIR / "07_after_client_a_give.png")
    # Client A a le coffre ouvert : il doit voir Pomme x3.
    phase("Assertion synchro", "client_a voit coffre à jour (Pomme x3)")
    assert_chest_apple_count("client_a", 3, "Après transfert, client_a doit voir 3 pommes dans le coffre.")

    phase("Client B ouvre le coffre", "scenario deterministe apres mise a jour du coffre")
    time.sleep(3.2)
    import_root(OUT_DIR / "08_client_b_chest_view.png")
    # Client B ouvre le coffre : les deux clients doivent voir le même coffre (Pomme x3).
    phase("Assertion synchro", "client_a et client_b voient le même coffre (Pomme x3)")
    assert_chest_sync_both_clients(3)
